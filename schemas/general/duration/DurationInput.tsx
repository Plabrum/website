...
import { type FieldMember, MemberField, type ObjectInputProps, type RenderFieldCallback } from 'sanity'

export function DurationInput(props: ObjectInputProps) {
  const { members, renderInput, renderItem, renderPreview } = props

  const fieldMembers = useMemo(() => members.filter(mem => mem.kind === 'field') as FieldMember[], [members])
