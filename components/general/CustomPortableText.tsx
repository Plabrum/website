...
export function CustomPortableText({ value }: { value: PortableTextBlock[] }) {
  ...
  const components: PortableTextComponents = {
    list: {
      ...
      bullet: ({ children }) => <ul className="mt-xl pl-10 list-disc marker:text-custom-t4">{children}</ul>,
      number: ({ children }) => <ol className="mt-lg pl-10 list-decimal">{children}</ol>,
    },

    block: {
      normal: ({ children }) => (
        <p className="sm:text-xl text-lg font-serif sm:leading-normal leading-snug my-2">{children}</p>
      ),
      h1: ({ children }) => <p className="sm:text-4xl text-3xl font-serif font-bold  mt-6 mb-4">{children}</p>,
      h2: ({ children }) => <p className="sm:text-3xl text-2xl font-serif text-center  mt-6 mb-4">{children}</p>,
      h3: ({ children }) => (
        <p className="mx-auto sm:text-3xl text-2xl text-center font-serif mt-6 mb-4 border-b border-custom-accent">
          {children}
        </p>
      ),
      h4: ({ children }) => <p className="mx-auto sm:text-2xl text-xl font-serif mt-6 mb-4">{children}</p>,
      blockquote: ({ children }) => (
